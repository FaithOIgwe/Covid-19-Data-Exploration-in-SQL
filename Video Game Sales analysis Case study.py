...
get_ipython().run_cell_magic('sql', '', '\n-- Select release year and average critic score for each year, rounded and aliased\n-- Join the game_sales and reviews tables\n-- Group by release year\n-- Order the data from highest to lowest avg_critic_score and limit to 10 results\n\nSELECT DISTINCT g.year, AVG(r.critic_score) AS avg_critic_score\nFROM games_sold AS g\nLEFT JOIN reviews AS r\nGROUP BY year\nORDER BY avg_critic_score DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', '\n-- Paste your query from the previous task; update it to add a count of games released in each year called num_games\n-- Update the query so that it only returns years that have more than four reviewed games\n\nSELECT g.year, AVG(r.critic_score) AS avg_critic_score, COUNT(year) AS num_games\nFROM games_sold AS g\nLEFT JOIN reviews AS r\nGROUP BY year\nWHERE year > 4\nORDER BY avg_critic_score DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', '\n-- Select the year results that appear on both tables\nSELECT year\nFROM top_critics_years_more_than_four_games\nINTERSECT\nSELECT year\nFROM top_user_years_more_than_four_games;')
